...
";
    ...
    if ($result_all->num_rows > 0) {
        while ($row = $result_all->fetch_assoc()) {
            ...
            echo '<a href="product.html?name=' . urlencode($row['name']) . '">';
            echo '<div class="transform rounded-lg bg-white p-4 shadow transition-transform duration-300 hover:scale-105 hover:shadow-lg">';
            echo '<div class="h-48 rounded bg-zinc-300" style="background-image: url(\'' . $row['image_url'] . '\'); background-size: cover; background-position: center;"></div>';
            ...
            if (!empty($salePrice)) {
                ...
                echo "<span class='ml-2 text-zinc-500 line-through'>$salePrice</span>";
            } else {
